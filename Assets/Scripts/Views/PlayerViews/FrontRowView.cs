...
public class FrontRowView : MonoBehaviour
{
    ...
    public static bool CursorOverSomeTable
    {
        get
        {
            FrontRowView[] bothTables = FindObjectsOfType<FrontRowView>();
    ...
    public int BoardPositionForNewCard(float MouseX)
    {
    ...
    {
        foreach (GameObject gameObject in cardsInRow)
        {
            if (gameObject != null)
            {
                gameObject.transform.DOLocalMoveX(slots.Children[cardsInRow.IndexOf(gameObject)].transform.localPosition.x, 0.3f);
            }
        }
    }
